...
public class IpUtil {
    ...
    public static boolean ipExistsInRange(String ip, String ipSection) {
        ...
        if (StringUtils.isNotBlank(ipSection)) {
            String[] ipSplit = ipSection.split(",");
            ip = ip.trim();
            for (int i = 0; i < ipSplit.length; i++) {
                String ipduan = ipSplit[i].trim();
                int idx = ipduan.indexOf('-');
                String beginIP = ipduan.substring(0, idx);
                String endIP = ipduan.substring(idx + 1);
    ...
    public static void main(String[] args) {
        ...
        String ip = "10.10.1.00";

        String ipSection = "101.123.123.234-123.123.123.123,10.10.1.00-10.10.10.00";
