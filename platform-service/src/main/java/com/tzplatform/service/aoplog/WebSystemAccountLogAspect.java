...
@Order(5)
@Component
public class WebSystemAccountLogAspect {
    ...
    @Pointcut("@annotation(com.tzplatform.utils.aoplog.AccountAopLog)")
    ...
    public void doAfterReturning(JoinPoint joinPoint, Object ret) throws Throwable {
        ...
        String account=request.getParameter(CommonEnum.REQUEST_ACCOUNT.用户账号.getValue());
        //如果账号为空,则从header获取token
        if(!StringUtils.isNotBlank(account)){
            String token = request.getHeader(CommonEnum.HEAD_PARAM.TOKEN.getValue());
            if(StringUtils.isNotBlank(token)){
                account = JwtUtils.veriftToken(token);
            }
        }
        //account不为空的情况下才记录日志
        if(StringUtils.isNotBlank(account)){
            ...
            PlatFormNormalLog platFormNormalLog = new PlatFormNormalLog(UidUtils.getId(), account, "", menuname, handleip, description, handleresult, new Date(), new Date());
    ...
    public void doafterThrowing(JoinPoint joinPoint, Throwable e) throws Throwable {
        ...
        if(StringUtils.isNotBlank(account)){
            PlatFormExceptionLog platFormExceptionLog = new PlatFormExceptionLog(UidUtils.getId(), account, handleip, "", menuname, description, methodname, errorinfo, new Date());
        ...
        for (Method method : methods) {
            if (method.getName().equals(methodName)) {
                ...
                if (clazzs.length == arguments.length) {
                    description = method.getAnnotation(AccountAopLog.class).description();
                    menuname = method.getAnnotation(AccountAopLog.class).menuname();
